...
from dataloader import VIMqDataset
from model import VIMqICModel

class Trainer(object):
    def __init__(self, config: VIMqConfig) -> None:
        super().__init__()
        self.config = config
        self.model = VIMqICModel(config=self.config)
        ...
        self.wsd_dataset = VIMqDataset(config=self.config, tokenizer=self.tokenizer)
